...
async def answer(
    ...
    try:
        ...
        if cached_result is not None:
            logger.info(f"Cache HIT: Using cached results for uploads hash: {cache_key[:8]}...")
            result = cached_result
        else:
            logger.info(f"Cache MISS: Processing documents for hash: {cache_key[:8]}...")
            ...
            await processed_docs_cache.set(
                cache_key, 
                result, 
                ttl=config.cache.DOCUMENT_PROCESS_CACHE_TTL
            )
